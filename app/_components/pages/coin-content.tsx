...
function ModerationDetail({ id }: { id: string }) {
...
export function CoinRouteContent({ path }: { path: string }) {
  ...
  if (path === "/coins/rules") return <RulesPage />;
  if (path === "/coins/economy") return <EconomyPage />;
  if (path === "/coins/bounties") return <BountiesPage />;
  if (path.startsWith("/coins/bounties/")) return <BountyDetail id={path.split("/")[3]} />;
  if (path === "/settings/coins") return <SettingsPage />;
  if (path === "/moderation/coins") return <ModerationPage />;
  if (path.startsWith("/moderation/coins/cases/")) return <ModerationDetail id={path.split("/")[4]} />;
