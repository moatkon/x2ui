...
const quests = [
  { id: "first-reply", node: "产品设计", title: "给一个无人回应的新主题提供首响", description: "先理解问题，再给出一个可执行的下一步。", reward: "8 CXP", progress: 1, max: 3, time: "约 12 分钟" },
  { id: "source-trail", node: "前端开发", title: "为高频问题补齐可信来源", description: "整理标准链接，并说明它解决了哪个具体疑问。", reward: "12 CXP", progress: 0, max: 4, time: "约 25 分钟" },
  { id: "thread-garden", node: "社区运营", title: "把一段分散讨论归纳成决策点", description: "保留分歧与来源，不替参与者改写观点。", reward: "16 CXP", progress: 2, max: 5, time: "约 35 分钟" },
];
...
function QuestList({ limit }: { limit?: number }) {
  return <ul className="list x2-list overflow-hidden rounded-box border-2 border-base-content/20">{quests.slice(0, limit ?? quests.length).map((quest) => <li className="list-row rounded-none" key={quest.id}><div className="list-col-grow min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="badge badge-outline">{quest.node}</span><span className="text-xs opacity-60">{quest.time}</span></div><Link className="mt-2 block font-bold hover:underline" href={`/quests/${quest.id}`}>{quest.title}</Link><p className="mt-1 text-sm opacity-65">{quest.description}</p><div className="mt-3"><ProgressBar value={quest.progress} max={quest.max} label="当前进度" /></div></div><span className="badge badge-success">{quest.reward}</span></li>)}</ul>;
}

function JourneyHub() {
  return <><PageHeader title="社区旅程" description="把阅读、回应和整理变成有边界的社区协作旅程。" action={<Link className="btn btn-primary" href="/quests/first-reply">继续当前任务</Link>} /><JourneyTabs active="hub" /><Notice kind="info"><p>游戏奖励贡献质量，不奖励发帖数量或在线时长；离线不会损失进度。</p></Notice><div className="mt-5"><StatGrid items={[{ title: "本周共建", value: "4 / 7", description: "覆盖 3 个节点" }, { title: "确认 CXP", value: 68, description: "另有 12 待确认" }, { title: "贡献角色", value: "回应者", description: "下一级：整理者" }]} /></div><Panel className="mt-5" title="当前旅程"><ul className="steps steps-vertical w-full sm:steps-horizontal"><li className="step step-primary">发现问题</li><li className="step step-primary">提供回应</li><li className="step">获得确认</li><li className="step">沉淀成果</li></ul></Panel><div className="mt-5"><QuestList limit={2} /></div></>;
}
...
function TeamsPage() {
  const teams = [["产品设计", "新人问题首响小队", "6 / 8 人", 7, 12], ["前端开发", "无障碍资料整理局", "8 / 10 人", 16, 24], ["社区运营", "案例归档协作局", "4 / 6 人", 3, 5]] as const;
  return <><PageHeader title="节点协作" description="小队围绕公开成果协作，不建立私聊、权力等级或无限排行。" /><JourneyTabs active="teams" /><Notice kind="info"><p>协作任务、验收规则和进度全部公开；成员可随时退出。</p></Notice><ul className="list x2-list mt-5 overflow-hidden rounded-box border-2 border-base-content/20">{teams.map(([node, title, members, value, max], index) => <li className="list-row rounded-none" key={title}><div className="list-col-grow"><div className="flex gap-2"><span className="badge badge-outline">{node}</span><span className="text-xs opacity-60">{members}</span></div><p className="mt-2 font-bold">{title}</p><div className="mt-3"><ProgressBar value={value} max={max} label="团队进度" /></div></div><ActionButton className="btn btn-sm" message={index ? "已提交加入申请" : "你已在这个节点协作局中"}>{index ? "申请加入" : "已加入"}</ActionButton></li>)}</ul></>;
}
...
function ContributionsPage() {
  return <><PageHeader title="我的贡献" description="从真实社区对象回看你产生的影响，不展示财富或名次。" /><StatGrid items={[{ title: "已确认贡献", value: 18, description: "永久记录" }, { title: "覆盖节点", value: 4, description: "最近 90 天" }, { title: "待质量确认", value: 2, description: "不计入已确认 CXP" }]} /><ul className="list x2-list mt-5 overflow-hidden rounded-box border-2 border-base-content/20">{[["有效首响", "前端开发", "帮助提问者补齐复现条件", "质量已确认"], ["讨论整理", "产品设计", "把 9 条评论归纳为 3 个决策点", "待确认"], ["规则引导", "社区运营", "为新成员指出节点边界", "质量已确认"]].map(([type, node, impact, status]) => <li className="list-row rounded-none" key={impact}><div className="list-col-grow"><div className="flex gap-2"><span className="badge badge-outline">{type}</span><span className="badge badge-info">{node}</span></div><p className="mt-2 font-bold">{impact}</p><p className="mt-1 text-sm opacity-60">{status}</p></div><Link className="btn btn-sm" href="/posts/immutable-content">查看来源</Link></li>)}</ul></>;
}

function CollectionPage() {
  const rows = [["回应者 · 路径印记", "完成 5 次经质量确认的有效回应"], ["开放资料索引 · 共建纪念", "参与前端开发节点公开资料第一期"], ["友善首响 · 展示徽记", "由被帮助者确认 3 次有效首响"]];
  return <><PageHeader title="贡献收藏" description="收藏是个人历程的纪念，不提供功能优势，也不能交易。" /><Notice kind="info"><p>所有收藏均可隐藏；展示不会影响内容排序、CXP、金币或治理权。</p></Notice><ul className="list x2-list mt-5 overflow-hidden rounded-box border-2 border-base-content/20">{rows.map(([name, description]) => <li className="list-row rounded-none" key={name}><div className="flex size-12 items-center justify-center rounded-full border-2 border-base-content/20">◇</div><div className="list-col-grow"><p className="font-bold">{name}</p><p className="mt-1 text-sm opacity-65">{description}</p></div><ActionButton className="btn btn-sm" dialogTitle="收藏展示设置" dialogBody={<p>是否展示只影响个人主页外观，不影响功能与排序。</p>}>展示设置</ActionButton></li>)}</ul></>;
}
...
export function JourneyRouteContent({ path }: { path: string }) {
  if (path === "/journey" || path === "/play") return <JourneyHub />;
  if (path === "/quests" || path === "/play/quests") return <QuestsPage />;
  if (path.startsWith("/quests/")) return <QuestDetail id={path.split("/")[2]} />;
  if (path.startsWith("/play/quests/")) return <QuestDetail id={path.split("/")[3]} />;
  if (path === "/me/progress" || path === "/play/journey") return <ProgressPage />;
  if (path === "/play/teams") return <TeamsPage />;
  if (path === "/journey/onboarding") return <OnboardingPage />;
  if (path === "/me/contributions") return <ContributionsPage />;
  if (path === "/me/collection") return <CollectionPage />;
  if (path === "/settings/journey") return <JourneySettingsPage />;
  if (path === "/journey/states") return <JourneyStatesPage />;
  return null;
}
