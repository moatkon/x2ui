...
function SettingsScaffold({ active, title, children }: { active: string; title: string; children: React.ReactNode }) {
  return <><PageHeader title="设置" description="管理公开资料、隐私、通知与账户安全。" /><PageTabs items={settingsTabs.map(([key, label]) => ({ label, href: `/settings/${key}`, active: key === active }))} /><Panel title={title}><SettingsForm>{children}</SettingsForm></Panel></>;
}

export function ProfileSettingsPage() {
  return <SettingsScaffold active="profile" title="编辑资料"><label className="form-control"><span className="label-text mb-2 font-semibold">显示名称</span><input className="input w-full" defaultValue="林默" required /></label><label className="form-control mt-4"><span className="label-text mb-2 font-semibold">个人简介</span><textarea className="textarea w-full" rows={4} maxLength={160} defaultValue="在复杂系统里寻找简单边界。" /><span className="label-text-alt mt-1 opacity-60">公开展示，最多 160 字。</span></label><label className="form-control mt-4"><span className="label-text mb-2 font-semibold">所在地</span><input className="input w-full" defaultValue="上海" /></label></SettingsScaffold>;
}

export function PrivacySettingsPage() {
  return <SettingsScaffold active="privacy" title="隐私设置"><div className="space-y-5">{[["允许出现在提及建议中", "被屏蔽的用户始终看不到你。", true], ["展示关注列表", "关闭后仅你自己可见。", true], ["搜索引擎收录个人页", "只影响公开个人页。", false]].map(([title, description, checked]) => <ToggleRow title={String(title)} description={String(description)} checked={Boolean(checked)} key={String(title)} />)}</div></SettingsScaffold>;
}

export function NotificationSettingsPage() {
  return <SettingsScaffold active="notifications" title="通知设置"><div className="space-y-5">{["回复与提及", "新关注", "内容互动", "治理进度", "账户安全"].map((title, index) => <ToggleRow title={title} description={index === 4 ? "安全通知不可完全关闭" : "站内通知与邮件摘要"} checked={index !== 2} disabled={index === 4} key={title} />)}</div></SettingsScaffold>;
}

export function SecuritySettingsPage() {
  return <SettingsScaffold active="security" title="账户安全"><Notice><p>账号邮箱：li•••@example.com（已验证）</p></Notice><div className="mt-5 divide-y-2 divide-base-content/20"><SettingLink title="登录密码" description="上次修改于 2026 年 4 月 18 日" href="/settings/security/password" label="修改密码" /><SettingLink title="设备会话" description="当前有 3 个有效会话" href="/settings/sessions" label="管理设备" /></div></SettingsScaffold>;
}

export function SessionSettingsPage() {
  return <SettingsScaffold active="sessions" title="设备会话"><DataTable headers={["设备", "位置", "最近活动", "操作"]} rows={[["Mac · Chrome（当前）", "上海", "刚刚", <span className="badge badge-success" key="current">当前</span>], ["iPhone · Safari", "上海", "昨天", <ActionButton className="btn btn-sm" message="设备会话已撤销" key="iphone">撤销</ActionButton>], ["Windows · Edge", "杭州", "7 月 4 日", <ActionButton className="btn btn-sm" message="设备会话已撤销" key="windows">撤销</ActionButton>]]} /><div className="mt-5"><ActionButton message="其他会话已全部撤销">撤销其他全部会话</ActionButton></div></SettingsScaffold>;
}
...
export function BookmarksPage() {
  return <><PageHeader title="我的收藏" description="保存下来，稍后继续阅读。" /><PostRows filter={(post) => posts.slice(0, 2).includes(post)} /></>;
}

export function FollowingPage() {
  return <><PageHeader title="关注 Feed" description="来自你关注的用户和节点，按时间排序。" /><PostRows filter={(post) => posts.slice(1).includes(post)} /></>;
}
