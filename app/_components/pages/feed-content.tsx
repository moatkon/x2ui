  ...
  return (
    <>
      ...
      <section className="mt-5 overflow-hidden rounded-box border-2 border-base-content/20">
        ...
        <nav className="p-3" aria-label="按一级节点浏览"><ul className="menu menu-horizontal flex w-full flex-wrap gap-1 px-0"><li><Link className={`content-center ${!parent ? "menu-active" : ""}`} href="/feed">全部</Link></li>{nodes.map((node) => <li key={node.id}><Link className={`content-center ${parent?.id === node.id ? "menu-active" : ""}`} href={`/feed?node=${node.slug}`}>{node.name}</Link></li>)}</ul></nav>
        {parent ? <nav className="border-t-2 border-base-content/20 p-3" aria-label={`${parent.name}下的主题`}><ul className="menu menu-horizontal flex flex-wrap gap-1 px-0"><li><Link className={`content-center ${!child ? "menu-active" : ""}`} href={`/feed?node=${parent.slug}`}>全部{parent.name}</Link></li>{parent.children.map((item) => <li key={item.id}><Link className={`content-center ${child?.id === item.id ? "menu-active" : ""}`} href={`/feed?node=${parent.slug}&subnode=${item.slug}`}>{item.name}</Link></li>)}</ul></nav> : null}
      </section>
