...
export function NodeDirectoryPage() {
...
export function ParentNodePage({ slug }: { slug: string }) {
  const parent = nodes.find((node) => node.slug === slug);
  if (!parent) notFound();
...
export function ChildNodePage({ parentSlug, childSlug }: { parentSlug: string; childSlug: string }) {
