...
export function ModerationRouteContent({ path }: { path: string }) {
  if (path === "/moderation" || path === "/moderation/cases") return <QueuePage />;
  if (path.startsWith("/moderation/cases/")) return <CasePage id={path.split("/")[3]} />;
  if (path === "/moderation/appeals") return <AppealsQueue />;
  if (path === "/moderation/audit" || path === "/moderation/audit-logs") return <AuditPage />;
  return null;
}
