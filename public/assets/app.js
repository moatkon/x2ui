(function () {
  ...
  document.addEventListener("click", e => {
    ...
    if (action === "quick-publish") {
      ...
      setTimeout(() => location.href = "/feed", 450);
    }
    ...
    if (action === "open-report") location.href = "/report/new";
    ...
    if (action === "confirm-bounty") { toast("悬赏已创建，金币已进入托管"); setTimeout(() => location.href = "/coins/bounties", 400); }
    ...
    if (action === "submit-report") { void mockWrite("POST", "/reports", { target: { type: "POST", id: "immutable-content" }, reason: "OTHER", details: "静态原型演示举报", evidenceUploadIds: [], truthfulConfirmation: true }, { idempotent: true }).catch(error => toast(error.message, "error")); toast("举报已提交，可在“我的举报”查看进度"); setTimeout(() => location.href = "/reports", 500); }
    if (action === "resume-bookmark") { state.bookmarked = true; toast("原任务已恢复：帖子已收藏"); setTimeout(() => location.href = "/posts/immutable-content", 500); }
    if (action === "change-password") { toast("密码已更新，可检查并撤销其他设备会话"); setTimeout(() => location.href = "/settings/sessions", 500); }
    if (action === "submit-appeal") { toast("申诉已提交"); setTimeout(() => location.href = "/appeals/AP-2026-0042", 500); }
    ...
    if (action === "finish-onboarding") { toast("旅程偏好已保存"); setTimeout(() => location.href = "/quests", 450); }
  ...
  document.addEventListener("submit", e => {
    ...
    if (e.target.dataset.form === "auth") {
      ...
      void mockWrite("POST", endpoint, payload, { idempotent: true }).then(() => {
        ...
        if (type === "forgot") { toast("如果邮箱已注册，将收到重置说明"); location.href = "/verify-email"; }
        else if (type === "verify") { toast("邮箱已验证"); location.href = "/verify-email/result"; }
        else if (type === "reset") { toast("密码已更新，请重新登录"); location.href = "/login"; }
        else { toast(type === "register" ? "账号已创建，请验证邮箱" : "登录成功，正在恢复来源任务"); location.href = type === "register" ? "/verify-email" : "/auth/recover-task"; }
      }).catch(error => toast(error.message, "error"));
    }
    if (e.target.dataset.form === "search") { e.preventDefault(); toast("已更新搜索结果", "info"); }
  });
  function requireLogin() { if (state.loggedIn) return true; sessionStorage.setItem("x2post-resume", `${location.pathname}${location.search}`); toast("请先登录，来源和输入已保存", "info"); setTimeout(() => location.href = "/auth/login", 350); return false; }
